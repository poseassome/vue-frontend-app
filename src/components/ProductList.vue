  ...
  <div id="productlist" class="container mx-auto mt-5" data-test="product-type">
    ...
    <ul class="px-5">
      <li v-for="productmenu in productmenus" :key="productmenu.id" class="my-2">
        ...
        query: { name: productmenu.name,
        img: productmenu.thumbnail,
        best: productmenu.best,
        desc: productmenu.description,
        price: productmenu.price }}">
          <div class="grid grid-cols-3">
              ...
              <div class="col-span-2 text-left">
                <p class="font-bold text-xl" data-test="product-name">{{ productmenu.name }}
                  <span v-if='productmenu.new==true' data-test="NewProduct"
                  class="text-green-700 text-sm align-super">New</span>
                  <span v-if='productmenu.popular==true' data-test="PopukarProduct"
                  class="text-red-700 text-sm align-super">Hot</span>
                </p>
                <p data-test="product-nameEn">{{ productmenu.nameEn }}</p>
...
export default {
  data() {
    return {
      productmenus: [
        {
          thumbnail: 'https://www.collinsdictionary.com/images/full/espresso_194276339.jpg',
          name: '에스프레소',
          nameEn: 'Espresso',
          best: false,
          new: false,
          popular: false,
          description: '향기로운 크레마 층과 바디층, 하트층으로 이루어져 있으며, 입안 가득히 커피와 달콤한 카라멜 향이 느껴지는 커피 음료',
          price: 4000,
        },
        {
          ...
          new: false,
          popular: true,
          description: '진한 에스프레소와 뜨거운 물을 섞어 깔끔하고 강렬한 에스프레소를 가장 부드럽게 잘 느낄 수 있는 커피',
          price: 4500,
        },
        {
